...
# cap = cv2.VideoCapture("../Videos/trafikOn4.mp4")   # Video kaynağı
cap = cv2.VideoCapture("../Videos/trafikArka2.mp4")   #Diger Video kaynağı
# #Cam
# cap = cv2.VideoCapture(0)
# cap.set(3, 1280)
# cap.set(4,720)

model = YOLO('../Yolo-Weights/yolov8n.pt')  # Kullanılan YOLO modelinin seçimi

# Sınıf adlarını tanımlama ve Mask kullanımı için
...
if totalCount:
    ...
    cursor.execute("INSERT INTO ArabaSayisi (KameraID, GecenArabaSayisi, KayitTarihi) VALUES (?, ?, GETDATE())", (1, vehicle_count))
